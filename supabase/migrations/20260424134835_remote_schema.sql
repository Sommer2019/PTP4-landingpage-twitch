...
CREATE OR REPLACE FUNCTION "clipvoting"."schedule_discord_notify"("p_endpoint" "text") RETURNS "void"
...
ALTER FUNCTION "clipvoting"."schedule_discord_notify"("p_endpoint" "text") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."clamp_points"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
BEGIN
...
ALTER FUNCTION "public"."clamp_points"() OWNER TO "postgres";
                                                                                     ...
                                                                                     v_reward IS NULL THEN 
                                                                                          RETURN jsonb_build_object('error', 'reward_not_found'); 
                                                                                           END IF; 
                                                                                                ...
                                                                                                -- Prüfe ob Reward deaktiviert ist 
...
CREATE TABLE IF NOT EXISTS "public"."bartclicker_scores" (
    ...
    "created_at" timestamp with time zone DEFAULT "now"(),
    "auto_click_buyer_unlocked" boolean DEFAULT false,
    "click_upgrade_buyer_unlocked" boolean DEFAULT false
);
...
ALTER TABLE ONLY "public"."donation_triggers"
    ADD CONSTRAINT "donation_triggers_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."donation_triggers"
    ADD CONSTRAINT "donation_triggers_trigger_id_key" UNIQUE ("trigger_id");
...
CREATE OR REPLACE TRIGGER "trg_clamp_points" BEFORE INSERT OR UPDATE ON "public"."points" FOR EACH ROW EXECUTE FUNCTION "public"."clamp_points"();
...
CREATE POLICY "Anyone can read donation_triggers" ON "public"."donation_triggers" FOR SELECT USING (true);
...
CREATE POLICY "Broadcaster kann Rewards einfügen" ON "public"."rewards" FOR INSERT WITH CHECK ((("current_setting"('request.jwt.claim.role'::"text", true) = 'service_role'::"text") OR (EXISTS ( SELECT 1
   FROM "public"."user_roles"
...
CREATE POLICY "Broadcaster kann Rewards ändern" ON "public"."rewards" FOR UPDATE USING ((("current_setting"('request.jwt.claim.role'::"text", true) = 'service_role'::"text") OR (EXISTS ( SELECT 1
   FROM "public"."user_roles"
  WHERE (("user_roles"."user_id" = "auth"."uid"()) AND ("user_roles"."is_broadcaster" = true)))) OR (EXISTS ( SELECT 1
   FROM "public"."user_roles"
...
CREATE POLICY "Moderators can delete donation_triggers" ON "public"."donation_triggers" FOR DELETE USING (("public"."is_moderator_role"() OR "public"."is_broadcaster_role"()));



CREATE POLICY "Moderators can insert donation_triggers" ON "public"."donation_triggers" FOR INSERT WITH CHECK (("public"."is_moderator_role"() OR "public"."is_broadcaster_role"()));



CREATE POLICY "Moderators can update donation_triggers" ON "public"."donation_triggers" FOR UPDATE USING (("public"."is_moderator_role"() OR "public"."is_broadcaster_role"())) WITH CHECK (("public"."is_moderator_role"() OR "public"."is_broadcaster_role"()));
...
GRANT ALL ON FUNCTION "public"."clamp_points"() TO "anon";
GRANT ALL ON FUNCTION "public"."clamp_points"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."clamp_points"() TO "service_role";
...
GRANT ALL ON TABLE "public"."donation_triggers" TO "anon";
GRANT ALL ON TABLE "public"."donation_triggers" TO "authenticated";
GRANT ALL ON TABLE "public"."donation_triggers" TO "service_role";
...
;
  ...
  create policy "Broadcaster can upload media"
  on "storage"."objects"
  as permissive
  for insert
  to authenticated
with check (((bucket_id = 'onlybart-media'::text) AND (EXISTS ( SELECT 1
   FROM public.user_roles
  WHERE ((user_roles.user_id = auth.uid()) AND (user_roles.is_broadcaster = true))))));



  create policy "Everyone can read media"
  on "storage"."objects"
  as permissive
  for select
  to public
using ((bucket_id = 'onlybart-media'::text));
